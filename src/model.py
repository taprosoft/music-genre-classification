...
from keras.layers import Input, Conv2D, Activation, BatchNormalization, Dense, Dropout, CuDNNLSTM
from keras.regularizers import l2
from keras import backend as K

from keras.layers import Dense, Dropout, Flatten, BatchNormalization, Input, concatenate
from keras.layers import Conv2D, MaxPooling2D, AveragePooling2D, ZeroPadding2D, Reshape
...
def simple_CNN(input_shape, num_classes):
    ...
    x = Conv2D(64, kernel_size=(input_shape[1], 4), activation='relu', input_shape=input_shape)(x)
    shortcut = x
    x = Conv2D(128, kernel_size=(1, 4), activation='relu', padding='same')(x)
    x = Conv2D(128, kernel_size=(1, 4), activation='relu', padding='same')(x)

    x = Conv2D(128, kernel_size=(1, 4), activation='relu', padding='same')(x)
    x = Conv2D(128, kernel_size=(1, 4), activation='relu', padding='same')(x)

    x1 = AveragePooling2D(pool_size=(1, 120))(concatenate([x, shortcut]))
    x2 = MaxPooling2D(pool_size=(1, 120))(concatenate([x, shortcut]))

    x = Dropout(0.1)(concatenate([x1, x2]))
    x = Flatten()(x)
